...
const target = path.resolve(websiteRoot, "public", "apps", "oido-absoluto-guitarra-juego");
const allowedRoot = path.resolve(websiteRoot, "public", "apps");

if (!target.startsWith(`${allowedRoot}${path.sep}`)) {
  throw new Error(`Destino fuera de public/apps: ${target}`);
}
