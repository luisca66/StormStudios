...
import { buildBlenderSchool, preloadBlenderSchool } from "../src/3d/creatures/blender-school";
...
controls.target.set(0,0,0);controls.minDistance=3;controls.maxDistance=15;controls.enableDamping=true;controls.update();
...
function reset() {
  ...
  const seventh=$<HTMLSelectElement>("palette").value==="SEVENTHS";
  creature=new Creature(buildBlenderSchool(FAMILY_GLOW[seventh?"SEVENTHS":"TRIADS"], $<HTMLSelectElement>("view").value === "single" ? 1 : 46),"school",CHORD_BY_ID[seventh?"MAJOR_7":"MAJOR"],"C4");
  creature.setHome(new THREE.Vector3());scene.add(creature.group);ending=false;
  status.value="Modelo de Blender listo · reposo";
}

try {
  await preloadBlenderSchool();reset();
  ...
  $("view").onchange=()=>{camera.position.set(5,2.5,8).multiplyScalar($<HTMLSelectElement>("view").value==="single"?.45:1);reset();}; $("reset").onclick=reset;$("palette").onchange=reset;
  const pause=$("pause");
  ...
  $("lighting").onchange=()=>{
    ...
    scene.background=new THREE.Color(light==="shallow"?0x2e86c1:light==="deep"?0x0b2438:0x061a27);
