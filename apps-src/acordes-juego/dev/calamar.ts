...
import { buildBlenderSquid, preloadBlenderSquid } from "../src/3d/creatures/blender-squid";
...
function reset() {
  ...
  const sixth=$<HTMLSelectElement>("palette").value==="SIXTHS";
  creature=new Creature(buildBlenderSquid(FAMILY_GLOW[sixth?"SIXTHS":"SEVENTHS"]),"squid",CHORD_BY_ID[sixth?"MAJOR_6":"DOMINANT_7"],"C4");
  creature.setHome(new THREE.Vector3());scene.add(creature.group);ending=false;
  status.value="Modelo de Blender listo · reposo";
}

try {
  await preloadBlenderSquid();reset();
