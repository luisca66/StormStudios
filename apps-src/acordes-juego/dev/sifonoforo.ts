...
import { buildBlenderSiphonophore, preloadBlenderSiphonophore } from "../src/3d/creatures/blender-siphonophore";
...
camera.position.set(9, 2, 11);
const renderer = new THREE.WebGLRenderer({canvas:document.querySelector("canvas")!,antialias:true});
renderer.setPixelRatio(Math.min(devicePixelRatio, 2));
const controls = new OrbitControls(camera, renderer.domElement);
controls.target.set(0,0,0);controls.minDistance=4;controls.maxDistance=40;controls.enableDamping=true;controls.update();
...
function reset() {
  ...
  creature=new Creature(buildBlenderSiphonophore(FAMILY_GLOW[thirteenth?"EXT_11_13":"SIXTHS"]),"siphonophore",CHORD_BY_ID[thirteenth?"MAJOR_13":"MAJOR_6"],"C4");
  creature.setHome(new THREE.Vector3());scene.add(creature.group);ending=false;
  status.value="Modelo de Blender listo · reposo";
}

try {
  await preloadBlenderSiphonophore();reset();
