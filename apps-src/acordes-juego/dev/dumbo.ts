...
import { buildBlenderDumbo, preloadBlenderDumbo } from "../src/3d/creatures/blender-dumbo";
...
function reset() {
  ...
  creature=new Creature(buildBlenderDumbo(FAMILY_GLOW[thirteenth?"EXT_11_13":"EXT_9"]),"dumbo",CHORD_BY_ID[thirteenth?"MAJOR_13":"MAJOR_9"],"C4");
  creature.setHome(new THREE.Vector3());scene.add(creature.group);ending=false;
  status.value="Modelo de Blender listo · reposo";
}

try {
  await preloadBlenderDumbo();reset();
