...
import { buildBlenderJellyfish, preloadBlenderJellyfish } from "../src/3d/creatures/blender-jellyfish";
...
camera.position.set(3, 1.7, 6);
const renderer = new THREE.WebGLRenderer({canvas:document.querySelector("canvas")!,antialias:true});
renderer.setPixelRatio(Math.min(devicePixelRatio, 2));
const controls = new OrbitControls(camera, renderer.domElement);
controls.target.set(0,-.4,0);controls.minDistance=3;controls.maxDistance=15;controls.enableDamping=true;controls.update();
...
function reset() {
  ...
  creature=new Creature(buildBlenderJellyfish(FAMILY_GLOW[seventh?"SEVENTHS":"TRIADS"]),"jellyfish",CHORD_BY_ID[seventh?"MAJOR_7":"MAJOR"],"C4");
  creature.setHome(new THREE.Vector3());scene.add(creature.group);ending=false;
  status.value="Modelo de Blender listo · reposo";
}

try {
  await preloadBlenderJellyfish();reset();
