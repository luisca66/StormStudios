"""Asteroides · Walking AP Multi, El Cosmos. Encargo de Gemini; receta en BRIEF-GEMINI.md de esta carpeta."""
...
try:
    parts, tris, size = kit.export_glb(ROOT / "asteroides-cosmos-juego.glb", objects=meshes, meta=meta,
                                       ao={"distance": 0.3, "strength": 0.6},
                                       json_path=ROOT / "asteroides-cosmos.json")
    ...
    if "Node" in str(e) or "node" in str(e) or "json-to-glb" in str(e):
        ...
        parts, tris = kit.export_parts(ROOT / "asteroides-cosmos.json", objects=meshes, meta=meta)
        size = (ROOT / "asteroides-cosmos.json").stat().st_size
    else:
        raise
...
for ob in meshes:
    ob.select_set(True)
bpy.ops.export_scene.gltf(filepath=str(ROOT / "asteroides-cosmos.glb"), export_format="GLB",
...
bpy.ops.wm.save_as_mainfile(filepath=str(ROOT / "asteroides-cosmos.blend"))
