# BRIEF-GEMINI — Asteroides · Walking AP Multi, nivel 3 «El Cosmos»
...
> (salida: `EXPORT 3 partes 1280 triangulos 38 KB`).

| | |
|---|---|
| Carpeta (única donde escribes) | `apps-src/oido-absoluto-multi-juego/art/blender/asteroides-cosmos/` |
| Dueño | Gemini |
| Script | `modelar-asteroides-cosmos.py` (desde la plantilla de `INSTRUCCIONES-GEMINI.md` §4) |
| Nombre del modelo en los archivos | `asteroides-cosmos` |
| Semilla | `SEED = 42` |

## 1. Qué es

Kit de **3 asteroides** para El Cosmos («cosmos de libro de cuentos: noche azul profunda»). El juego
reparte unos 40 por el nivel, grandes (radio 2–8.5 m, los escala él), girando despacio; son obstáculos.
Estilo **low-poly facetado** (caras planas), roca azul pizarra que se lee sobre el fondo azul medianoche.
...
| `meta` | `dict(forward="+Z", note="kit instanciado; pivote en el centro; el juego escala a radio 2-8.5 m")` |
...
- [ ] Caras planas visibles (facetado), color azul pizarra; no blancos ni grises neutros.
- [ ] Sin suelo en la escena exportada.

## 8. Entregables

`modelar-asteroides-cosmos.py`, `asteroides-cosmos.blend`, `asteroides-cosmos.glb`, `asteroides-cosmos-juego.glb`,
`asteroides-cosmos.json`, `render-kit.png` y `ENTREGA.md` (`Lista para: revisión`). Nada más en la carpeta.
