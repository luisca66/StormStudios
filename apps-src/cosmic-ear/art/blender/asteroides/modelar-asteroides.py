"""Asteroides · Cosmic Ear. Encargo de Gemini; receta en BRIEF-GEMINI.md de esta carpeta."""
...
try:
    parts, tris, size = kit.export_glb(ROOT / "asteroides-juego.glb", objects=meshes, meta=meta,
                                       ao={"distance": 0.3, "strength": 0.6},
                                       json_path=ROOT / "asteroides.json")
    ...
    if "Node" in str(e) or "node" in str(e) or "json-to-glb" in str(e):
        ...
        parts, tris = kit.export_parts(ROOT / "asteroides.json", objects=meshes, meta=meta)
        size = (ROOT / "asteroides.json").stat().st_size
    else:
        raise
...
for ob in meshes:
    ob.select_set(True)
bpy.ops.export_scene.gltf(filepath=str(ROOT / "asteroides.glb"), export_format="GLB",
...
bpy.ops.wm.save_as_mainfile(filepath=str(ROOT / "asteroides.blend"))
