...
//
// Tres almacenes SEPARADOS de los de la webapp seria (prefijo `expreso-`), como manda
// §7.7. Se guarda tras CADA decisión, no al final del viaje: si el navegador se cierra a
// mitad de ruta, las estadísticas de los grados ya respondidos no se pierden.
//
// Regla de oro del tablero: los récords SOLO MEJORAN. Un viaje malo en una ruta ya
// conquistada no puede borrar la medalla de oro que costó ganar.

import type { Degree } from "@/music/degrees";

const KEY_STATS = "expreso-stats";
const KEY_ROUTES = "expreso-rutas";
const KEY_SETTINGS = "expreso-settings";
...
/**
 * Registra una llegada. `mejorMedalla`, `mejorScore` y `mejorRacha` SOLO MEJORAN, y
 * `velocidadRecord` acompaña siempre a la puntuación récord (si no, diría la velocidad
 * de la última partida, que puede no tener nada que ver con el récord).
 */
...
/** Borra TODO el progreso del Expreso. La confirmación es cosa de la UI. */
