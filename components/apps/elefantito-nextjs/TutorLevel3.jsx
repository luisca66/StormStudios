...
const TX = {
  es: {
    ...
    intro_body:   "Separa ambos números en decenas y unidades. Suma las decenas primero, luego las unidades, luego combínalas.",
    ex_label:     "— EJEMPLO —",
    ex_step1:     "60 + 20  =  80",
    ex_step2:     " 7 +  8  =  15",
    ex_step3:     "80 + 15  =  95",
    ...
    step:         (i) => `PASO ${i}`,
    hint_label:   "PISTA:",
    hint_0:       "Ignora las unidades. Suma solo las decenas de ambos números.",
    hint_1:       "Ignora las decenas. Suma solo las unidades de ambos números.",
    hint_2:       (ts, os) => `Junta los dos resultados: ${ts} + ${os}.`,
    btn_check:    "VERIFICAR",
    btn_done:     "¡Entendido, estoy listo!",
    step1_ins:    (at, bt)    => `Suma las decenas (${at} + ${bt})`,
    step2_ins:    (ao, bo)    => `Suma las unidades (${ao} + ${bo})`,
    step3_ins:    (ts, os)    => `Suma ambos resultados (${ts} + ${os})`,
    sound_on:     "🔊 Sonido ON",
  ...
  en: {
    ...
    intro_body:   "Split both numbers into tens and ones. Add the tens first, then the ones, then combine them.",
    ex_label:     "— EXAMPLE —",
    ex_step1:     "60 + 20  =  80",
    ex_step2:     " 7 +  8  =  15",
    ex_step3:     "80 + 15  =  95",
    ...
    step:         (i) => `STEP ${i}`,
    hint_label:   "HINT:",
    hint_0:       "Ignore the ones digits. Add only the tens of both numbers.",
    hint_1:       "Ignore the tens digits. Add only the ones of both numbers.",
    hint_2:       (ts, os) => `Combine the two results: ${ts} + ${os}.`,
    btn_check:    "CHECK",
    btn_done:     "Got it, I'm ready!",
    step1_ins:    (at, bt)    => `Add the tens (${at} + ${bt})`,
    step2_ins:    (ao, bo)    => `Add the ones (${ao} + ${bo})`,
    step3_ins:    (ts, os)    => `Add both results (${ts} + ${os})`,
    sound_on:     "🔊 Sound ON",
    sound_off:    "🔇 Sound OFF",
  },
};
...
function genProblem() {
  let a = Math.floor(Math.random() * 79) + 11;
  let b = Math.floor(Math.random() * 79) + 11;
  while (a % 10 === 0) a = Math.floor(Math.random() * 79) + 11;
  while (b % 10 === 0) b = Math.floor(Math.random() * 79) + 11;
  ...
  return { a, b, aTens, aOnes, bTens, bOnes, tensSum, onesSum, answer: a + b };
}

export default function TutorLevel3({ onComplete }) {
  ...
  // Focus input when step or problem changes
  useEffect(() => {
  ...
  const steps = [
    {
      instruction: tx.step1_ins(problem.aTens, problem.bTens),
      answer:      problem.tensSum,
      hint:        tx.hint_0,
    },
    {
      instruction: tx.step2_ins(problem.aOnes, problem.bOnes),
      answer:      problem.onesSum,
      hint:        tx.hint_1,
    },
    {
      instruction: tx.step3_ins(problem.tensSum, problem.onesSum),
      answer:      problem.answer,
      hint:        tx.hint_2(problem.tensSum, problem.onesSum),
    },
  ];
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-5 max-w-md mx-auto w-full py-4">
          ...
          <div className="bg-[#060810] border-2 border-[#00eeff40] rounded-sm w-full shadow-[0_0_16px_rgba(0,238,255,0.06)]">
            <div className="border-b border-[#00eeff20] px-4 py-2 flex items-center justify-between">
              ...
              <span className="text-[#ffe600] text-[0.65rem] drop-shadow-[0_0_6px_rgba(255,230,0,0.4)]">67 + 28</span>
            </div>
  ...
  return (
    <div className="bg-[#0c0e1a] border-2 border-[#14161e] p-4 flex flex-col h-[80vh] font-[family-name:var(--font-press-start-2p)] overflow-hidden">
      ...
      <div className="bg-[#060810] border-[3px] border-[#ffe600] shadow-[0_0_0_3px_#000,0_0_12px_rgba(255,230,0,0.08)] rounded-sm p-4 mb-4 text-center shrink-0">
        <div className="text-[#55555a] text-[0.54rem] tracking-[0.2em] mb-2">SUMA</div>
          ...
          return (
            <div key={idx} className={`flex items-center justify-between px-4 py-3 rounded-sm border-2 transition-all ${
              ...
              <div className="flex items-center gap-3 min-w-0">
                <span className={`text-[0.52rem] shrink-0 ${isAnswered ? "text-[#39ff1470]" : isCurrent ? "text-[#00eeff70]" : "text-[#202020]"}`}>
                  {tx.step(idx + 1)}
                </span>
                <span className="text-[0.6rem] leading-[1.8]">{s.instruction}</span>
              </div>
              {isAnswered && (
      ...
      {errors >= 2 && !allStepsDone && (
        <div className="bg-[#0f0a00] border border-[#ffe60040] px-4 py-3 mb-3 shrink-0">
          <span className="text-[#ffe600b0] text-[0.56rem] leading-[1.8]">
            {tx.hint_label}{" "}{steps[step].hint}
          </span>
        </div>
      )}

      {/* Ready button — always visible, user decides */}
