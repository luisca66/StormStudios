...
const TX = {
  es: {
    intro_title:  "RESTA",
    intro_sub:    "2 CIFRAS · SIN PRÉSTAMO",
    intro_body:   "Resta dígito a dígito de izquierda a derecha. Primero quitas las decenas y luego las unidades. Solo funciona cuando cada dígito del número de abajo es menor o igual al de arriba.",
    ex_label:     "— EJEMPLO —",
    ex_step1:     "86 − 20  =  66",
    ex_step2:     "66 −  3  =  63",
    ex_result:    "86 − 23  =  63",
    ...
    step:         (i) => `PASO ${i}`,
    hint_label:   "PISTA:",
    hint_0:       (a, bT)       => `Ignora las unidades. Solo resta las decenas: ${a} − ${bT}.`,
    hint_1:       (s1, bO)      => `El paso anterior dio ${s1}. Ahora réstale las unidades: ${s1} − ${bO}.`,
    btn_check:    "VERIFICAR",
    btn_done:     "¡Entendido, estoy listo!",
    step1_ins:    (a, bT)       => `Resta las decenas (${a} − ${bT})`,
    step2_ins:    (s1, bO)      => `Resta las unidades (${s1} − ${bO})`,
    sound_on:     "🔊 Sonido ON",
    sound_off:    "🔇 Sonido OFF",
  },
  en: {
    intro_title:  "SUBTRACTION",
    intro_sub:    "2 DIGITS · NO BORROWING",
    intro_body:   "Subtract digit by digit, left to right. First remove the tens, then the ones. This works whenever each digit of the bottom number is less than or equal to the one above it.",
    ex_label:     "— EXAMPLE —",
    ex_step1:     "86 − 20  =  66",
    ex_step2:     "66 −  3  =  63",
    ex_result:    "86 − 23  =  63",
    ...
    step:         (i) => `STEP ${i}`,
    hint_label:   "HINT:",
    hint_0:       (a, bT)       => `Ignore the ones. Just subtract the tens: ${a} − ${bT}.`,
    hint_1:       (s1, bO)      => `The previous step gave ${s1}. Now subtract the ones: ${s1} − ${bO}.`,
    btn_check:    "CHECK",
    btn_done:     "Got it, I'm ready!",
    step1_ins:    (a, bT)       => `Subtract the tens (${a} − ${bT})`,
    step2_ins:    (s1, bO)      => `Subtract the ones (${s1} − ${bO})`,
    sound_on:     "🔊 Sound ON",
    sound_off:    "🔇 Sound OFF",
  },
};
...
function genProblem() {
  let a, b, bTens, bOnes, step1, answer;
  do {
    a     = Math.floor(Math.random() * 70) + 30;          // 30–99
    const aTens = Math.floor(a / 10);
    const aOnes = a % 10;
    if (aOnes === 0) continue;                             // need ones ≥ 1
    bTens = (Math.floor(Math.random() * aTens) + 1) * 10; // 10…aTens×10
    bOnes = Math.floor(Math.random() * aOnes) + 1;        // 1…aOnes
    b     = bTens + bOnes;
    step1  = a - bTens;
    answer = step1 - bOnes;
  } while (b >= a || answer <= 0);

  return { a, b, bTens, bOnes, step1, answer };
}

export default function TutorLevel7({ onComplete }) {
  ...
  const steps = [
    {
      instruction: tx.step1_ins(problem.a, problem.bTens),
      answer:      problem.step1,
      hint:        tx.hint_0(problem.a, problem.bTens),
    },
    {
      instruction: tx.step2_ins(problem.step1, problem.bOnes),
      answer:      problem.answer,
      hint:        tx.hint_1(problem.step1, problem.bOnes),
    },
  ];
