...
const TX = {
  es: {
    ...
    intro_sub:    "A 100 · DOS CIFRAS",
    intro_body:   "El complemento a 100 de un número de 2 cifras se calcula dígito a dígito: al primero le falta para llegar a 9, al último le falta para llegar a 10.",
    ex_label:     "— EJEMPLO —",
    ex_step1:     "9 − 3  =  6",
    ex_step2:     "10 − 7  =  3",
    ex_step3:     "Une:  6 y 3  →  63",
    ex_result:    "Compl. de 37  =  63",
    btn_start:    "PRACTICAR →",
    solved:       (n) => `Resueltos: ${n}`,
    step:         (i) => `PASO ${i}`,
    hint_label:   "PISTA:",
    hint_0:       (d1)     => `¿Cuánto falta para llegar a 9? ${d1} + ? = 9.`,
    hint_1:       (d2)     => `¿Cuánto falta para llegar a 10? ${d2} + ? = 10.`,
    hint_2:       (s1, s2) => `Pon el ${s1} seguido del ${s2}: ${s1}${s2}.`,
    btn_check:    "VERIFICAR",
    btn_done:     "¡Entendido, estoy listo!",
    step1_ins:    (d1)     => `¿Cuánto le falta al primer dígito (${d1}) para llegar a 9?`,
    step2_ins:    (d2)     => `¿Cuánto le falta al último dígito (${d2}) para llegar a 10?`,
    step3_ins:    (s1, s2) => `Une ambos resultados (${s1} y ${s2})`,
    sound_on:     "🔊 Sonido ON",
  ...
  en: {
    ...
    intro_sub:    "TO 100 · TWO DIGITS",
    intro_body:   "The complement to 100 of a 2-digit number is found digit by digit: the first needs to reach 9, the last needs to reach 10.",
    ex_label:     "— EXAMPLE —",
    ex_step1:     "9 − 3  =  6",
    ex_step2:     "10 − 7  =  3",
    ex_step3:     "Join:  6 and 3  →  63",
    ex_result:    "Compl. of 37  =  63",
    btn_start:    "PRACTICE →",
    solved:       (n) => `Solved: ${n}`,
    step:         (i) => `STEP ${i}`,
    hint_label:   "HINT:",
    hint_0:       (d1)     => `How much to reach 9? ${d1} + ? = 9.`,
    hint_1:       (d2)     => `How much to reach 10? ${d2} + ? = 10.`,
    hint_2:       (s1, s2) => `Place ${s1} then ${s2}: ${s1}${s2}.`,
    btn_check:    "CHECK",
    btn_done:     "Got it, I'm ready!",
    step1_ins:    (d1)     => `How much does the first digit (${d1}) need to reach 9?`,
    step2_ins:    (d2)     => `How much does the last digit (${d2}) need to reach 10?`,
    step3_ins:    (s1, s2) => `Join both results (${s1} and ${s2})`,
    sound_on:     "🔊 Sound ON",
    sound_off:    "🔇 Sound OFF",
    label:        "COMPL. OF",
  },
...
function genProblem() {
  let n;
  do {
    n = Math.floor(Math.random() * 89) + 10; // 10–98
  } while (n % 10 === 0);                    // exclude multiples of 10

  const d1    = Math.floor(n / 10); // tens digit
  const d2    = n % 10;             // ones digit
  const step1 = 9 - d1;             // tens complement (0–8)
  const step2 = 10 - d2;            // ones complement (1–9)
  const answer = step1 * 10 + step2; // combined

  return { n, d1, d2, step1, step2, answer };
}
...
export default function TutorLevel10({ onComplete }) {
  ...
  const steps = [
    ...
    {
      instruction: tx.step3_ins(problem.step1, problem.step2),
      answer:      problem.answer,
      hint:        tx.hint_2(problem.step1, problem.step2),
    },
  ];
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-5 max-w-md mx-auto w-full py-4">
          ...
          <div className="bg-[#060810] border-2 border-[#b026ff40] rounded-sm w-full shadow-[0_0_16px_rgba(176,38,255,0.06)]">
            <div className="border-b border-[#b026ff20] px-4 py-2 flex items-center justify-between">
              ...
              <span className="text-[#ffe600] text-[0.65rem] drop-shadow-[0_0_6px_rgba(255,230,0,0.4)]">37</span>
            </div>
