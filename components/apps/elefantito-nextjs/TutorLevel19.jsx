...
const TX = {
  es: {
    intro_title: "DIVISION: 3 CIFRAS",
    intro_sub: "COCIENTE DE 3 CIFRAS",
    intro_body: "La tecnica es la misma que en el nivel anterior, pero ahora sostienes tres pasos de cociente. Encuentra un digito, conserva el residuo, baja el siguiente digito y repite.",
    ex_label: "- EJEMPLO -",
    ex_step1: "6 cabe en 45: 7 veces (42)",
    ex_step2: "45 - 42 = 3",
    ex_step3: "Baja 7: 37. 6 cabe 6 veces (36)",
    ex_step4: "37 - 36 = 1",
    ex_step5: "Baja 9: 19. 6 cabe 3 veces (18)",
    ex_step6: "19 - 18 = 1",
    ex_result: "Resultado: 763 con residuo 1",
    ex_rule: "La clave es no soltar el residuo: cada residuo se une al siguiente digito.",
    btn_start: "PRACTICAR ->",
    solved: (n) => `Resueltos: ${n}`,
    step: (i) => `PASO ${i}`,
    prompt_digit: (d, n) => `¿Cuántas veces cabe ${d} en ${n}?`,
    prompt_remainder: (n, product) => `Residuo: ${n} - ${product} =`,
    prompt_result: (q, r) => `Resultado: ${q} con residuo ${r}`,
    hint_label: "PISTA:",
    hint_digit: "Busca solo el siguiente digito del cociente.",
    hint_remainder: "Resta el producto y conserva el residuo para bajar el siguiente digito.",
    hint_result: "Junta los tres digitos del cociente; el residuo queda aparte.",
    btn_check: "VERIFICAR",
  ...
  en: {
    intro_title: "DIVISION: 3 DIGITS",
    intro_sub: "THREE-DIGIT QUOTIENT",
    intro_body: "The technique is the same as the previous level, but now you hold three quotient steps. Find one digit, keep the remainder, bring down the next digit, and repeat.",
    ex_label: "- EXAMPLE -",
    ex_step1: "6 goes into 45: 7 times (42)",
    ex_step2: "45 - 42 = 3",
    ex_step3: "Bring down 7: 37. 6 goes 6 times (36)",
    ex_step4: "37 - 36 = 1",
    ex_step5: "Bring down 9: 19. 6 goes 3 times (18)",
    ex_step6: "19 - 18 = 1",
    ex_result: "Result: 763 remainder 1",
    ex_rule: "The key is not dropping the remainder: each remainder joins the next digit.",
    btn_start: "PRACTICE ->",
    solved: (n) => `Solved: ${n}`,
    step: (i) => `STEP ${i}`,
    prompt_digit: (d, n) => `How many times does ${d} fit into ${n}?`,
    prompt_remainder: (n, product) => `Remainder: ${n} - ${product} =`,
    prompt_result: (q, r) => `Result: ${q} remainder ${r}`,
    hint_label: "HINT:",
    hint_digit: "Find only the next digit of the quotient.",
    hint_remainder: "Subtract the product and keep the remainder before bringing down the next digit.",
    hint_result: "Join the three quotient digits; the remainder stays separate.",
    btn_check: "CHECK",
...
function buildDivision(dividend, divisor) {
  ...
  while (current < divisor && index < digits.length - 1) {
    index += 1;
  ...
  while (index < digits.length) {
    ...
    quotientText += String(digit);
    steps.push({ current, digit, product, remainder });

    index += 1;
    if (index < digits.length) current = remainder * 10 + digits[index];
  }

  return {
    quotient: Number(quotientText),
...
function genProblem() {
  let divisor, quotient, remainder, dividend, division;
  ...
  do {
    ...
    quotient = hundreds * 100 + tens * 10 + units;
    remainder = Math.floor(Math.random() * divisor);
    dividend = quotient * divisor + remainder;
    division = buildDivision(dividend, divisor);
  } while (division.steps.length !== 3 || division.quotient !== quotient);

  return { divisor, quotient, remainder, dividend, divisionSteps: division.steps };
}

export default function TutorLevel19({ onComplete }) {
  ...
  const divisionSteps = problem.divisionSteps;
  const steps = [
    {
      instruction: tx.prompt_digit(problem.divisor, divisionSteps[0].current),
      answer: divisionSteps[0].digit,
      hint: tx.hint_digit,
    },
    {
      instruction: tx.prompt_remainder(divisionSteps[0].current, divisionSteps[0].product),
      answer: divisionSteps[0].remainder,
      hint: tx.hint_remainder,
    },
    {
      instruction: tx.prompt_digit(problem.divisor, divisionSteps[1].current),
      answer: divisionSteps[1].digit,
      hint: tx.hint_digit,
    },
    {
      instruction: tx.prompt_remainder(divisionSteps[1].current, divisionSteps[1].product),
      answer: divisionSteps[1].remainder,
      hint: tx.hint_remainder,
    },
    {
      instruction: tx.prompt_digit(problem.divisor, divisionSteps[2].current),
      answer: divisionSteps[2].digit,
      hint: tx.hint_digit,
    },
    {
      instruction: tx.prompt_remainder(divisionSteps[2].current, divisionSteps[2].product),
      answer: divisionSteps[2].remainder,
      hint: tx.hint_remainder,
    },
    {
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-5 max-w-xl mx-auto w-full py-4">
          ...
          <div className="bg-[#060810] border-2 border-[#b026ff40] rounded-sm w-full shadow-[0_0_16px_rgba(176,38,255,0.06)]">
            <div className="border-b border-[#b026ff20] px-4 py-2 flex items-center justify-between">
              ...
              <span className="text-[#ffe600] text-[0.65rem] drop-shadow-[0_0_6px_rgba(255,230,0,0.4)]">4579 / 6</span>
            </div>
            <div className="p-4 flex flex-col gap-1.5">
              ...
              <div className="text-[#39ff14] text-[0.56rem] leading-[2] drop-shadow-[0_0_6px_rgba(57,255,20,0.3)]">
                {tx.ex_rule}
              </div>
            </div>
          </div>
