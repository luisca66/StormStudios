...
import { translateLessonSlug, translateResourceSlug } from "@/data/seo/localized-slugs";
    ...
    startTransition(() => {
      if (isDynamicPathname(pathname) && currentSlug) {
        ...
        if (pathname === "/resources/[slug]") {
          nextSlug = translateResourceSlug(currentLocale, currentSlug, nextLocale) ?? currentSlug;
        }

        if (pathname === "/curso-armonia/[slug]") {
          nextSlug = translateLessonSlug(currentLocale, currentSlug, nextLocale) ?? currentSlug;
        }
